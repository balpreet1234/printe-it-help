<footer class="footer_bg_img pt-5 pb-2 position-relative">
    <div class="footer_bg_layer z-0 pointer_events"></div>
    <div class="container position-relative z-1">
        <div class="row justify-content-between align-items-center">
            <div class="col-lg-4 col-12">
                <div class="bg-white page_logo mb-2 px-2 py-1 border_radius_6">
                    <a href="#"><img src="{{ asset('public/assets/images/png/page-logo.png') }}" alt="page-logo"
                            class="page_logo w-100"></a>
                </div>
                <p class="text-white fs_16 ">Lorem, ipsum dolor sit amet consectetur adipisicing
                    elit.
                    Esse,
                    reprehenderit! Lorem ipsum dolor sit amet, consectetur adipisicing. Lorem, ipsum.</p>
                <div class="d-flex gap-3">
                    <a href="#"><img src="{{ asset('public/assets/images/png/instagram.png') }}" alt="instagram"
                            class="social_icon"></a>
                    <a href="#"><img src="{{ asset('public/assets/images/png/whatsapp.png') }}" alt="whatsapp"
                            class="social_icon"></a>
                    <a href="#"><img src="{{ asset('public/assets/images/png/facebook.png') }}" alt="facebook"
                            class="social_icon"></a>
                </div>
            </div>
            <div class="col-xl-6 col-lg-7 col-12 mt-4 mt-lg-0">
                <div class="row align-items-center">
                    <div class="col-sm-6 col-12">
                        <ul class="p-0 m-0">
                            <li class="text-white fs_18 fw-medium mb-2">Our Services</li>
                            <li><a href="#"
                                    class="fw-normal text-white mb-2 d-inline-block footer_link position-relative">
                                    HP
                                    Printer Support Services</a></li>
                            <li><a href="#"
                                    class="fw-normal text-white mb-2 d-inline-block footer_link position-relative">
                                    HP
                                    Printer Support Services</a></li>
                            <li><a href="#"
                                    class="fw-normal text-white mb-2 d-inline-block footer_link position-relative">
                                    HP
                                    Printer Support Services</a></li>
                            <li><a href="#"
                                    class="fw-normal text-white mb-2 d-inline-block footer_link position-relative">
                                    HP
                                    Printer Support Services</a></li>
                        </ul>
                    </div>
                    <div class="col-sm-6 col-12 mt-3 mt-sm-0">
                        <p class="text-white fs_18 fw-medium mb-2">Contact Us</p>
                        <div class="d-flex gap-3 align-items-center pe-lg-5">
                            <img src="{{ asset('public/assets/images/png/location-pin.png') }}" alt="location-pin" class="mail_icon">
                            <div>
                                <p class="text-white fw-medium fs_18 mb-0">Location</p>
                                <a href="#" class="text-white fs_16 fw-normal">Lorem,
                                    ipsum dolor.</a>
                            </div>
                        </div>
                        <div class="d-flex gap-3 align-items-center mt-3">
                            <img src="{{ asset('public/assets/images/png/email.png') }}" alt="mail-icon" class="mail_icon">
                            <div>
                                <p class="text-white fw-medium fs_18 mb-0">Mail Us</p>
                                <a href="#" class="text-white fs_16 fw-normal">contactus@gmail.com</a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="footer_line mt-5 position-relative z-1"></div>
    <p class="fw-normal text-white text-center fs_16 my-3 opacity-75 position-relative z-1">Lorem
        ipsum dolor sit, amet
        consectetur adipisicing.</p>
</footer>
